...
class BoxRFDGNN:
    def __init__(self, config_path=None, model_path=None, imf_model_path=None, table_grid_path=None, feature_set_name='imf+rf',
        ...
        self._enable_inference_cache = enable_inference_cache
        self._predict_cache = None  # cached raw groups from last predict()
        self._predict_cache_page_id = None  # (doc.name, page.number) of the cached page
    ...
    def predict(self, page, verbose=False, **kwargs):
        ...
        # Check cached result
        # The cache stores post-processed groups with potentially updated bboxes and table grids.
        if (self._enable_inference_cache
                and self._predict_cache_page_id is not None
                and (page.parent.name, page.number) == self._predict_cache_page_id):
            groups = self._predict_cache
        ...
        # Update cache with post-processing enriched groups
        # This cache now holds groups with final bboxes and table structures.
        if self._enable_inference_cache:
            self._predict_cache_page_id = (page.parent.name, page.number)
            self._predict_cache = groups
